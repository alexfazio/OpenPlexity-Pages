...
with left_column:
    ...
    if generation_details[f"toggle_audience"]:
        generation_details[f"audience"] = st.selectbox("Audience",
    ...
    if generation_details[f"toggle_role"]:
        generation_details[f"role"] = st.selectbox("Role", ["William Shakespeare", "George R.R. Martin", "Tolkien"],
                                                   key=f"role")
    else:
    ...
    for block in story_blocks:
        with st.expander(f"{block} Block", expanded=True):
            generation_details[f"{block}_block_title"] = st.text_input(f"{block} Block Title", f"{block}",
                                                                       key=f"{block}_title")
            generation_details[f"{block}_word_count"] = st.slider("Word Count", min_value=100, max_value=1000,
                                                                  value=300, step=50, key=f"{block}_word_count")
            generation_details[f"{block}_sentence_count"] = generation_details[
                                                                f"{block}_word_count"] / WORDS_PER_SENTENCE
            generation_details[f"{block}_keywords"] = st.text_input("Keywords", key=f"{block}_keywords")
            st.divider()
            generation_details[f"{block}_llm_model"] = st.selectbox("LLM Model", ["Mistral", "GPT-3", "GPT-4", "Claude",
                                                                                  "Custom Model"],
                                                                    key=f"{block}_llm_model")
            generation_details[f"{block}_temperature"] = st.slider("Temperature", min_value=0.0, max_value=1.0,
                                                                   value=0.7, step=0.1, key=f"{block}_temperature")
...
with middle_column:
    ...
    for block in story_blocks:
        if f"{block}_prompt" in st.session_state:
            ...
            st.text_area(f"{block} Prompt", st.session_state[f"{block}_prompt"], height=300,
